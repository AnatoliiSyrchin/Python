...
class Storage:
    ...
    def move_to(self, target_obj, *equipment):
        ...
        for i in range(int(printers)):
            target_obj.storage['Printer'].append(self.storage['Printer'].pop())
    ...
    def validate_equipment(self, printers, scanners, copiers):
        ...
        if int(printers) > len(self.storage['Printer']) or int(scanners) > len(self.storage['Scanner']) or int(
                copiers) > len(self.storage['Copier']):
